...
async function updateProductAttributes(attributes, productId, variantGroupId, connection) {
    if (!variantGroupId) {
        ...
        const variantGroup = await select('attribute_one', 'attribute_two', 'attribute_three', 'attribute_four', 'attribute_five')
            .from('variant_group')
        ...
        const variantAttributes = await select()
            .from('attribute')
            .where('attribute_id', 'IN', Object.values(variantGroup).filter((v) => v !== null))
            .execute(connection);
...
async function updateProduct(uuid, data, context) {
    ...
    try {
        ...
        // Update product inventory
        await hookable(updateProductInventory, { ...context, connection, product })(productData, product.product_id, connection);
        ...
        await hookable(updateProductImages, { ...context, connection, product })(productData.images, product.product_id, connection);
