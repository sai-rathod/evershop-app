...
const buildUrlReWrite = async (data) => {
    try {
      ...
      INNER JOIN parent_categories pc ON c.category_id = pc.parent_id
        ...
        if (currentPath) {
            await execute(pool, `UPDATE url_rewrite SET request_path = REPLACE(request_path, '${currentPath.request_path}', '${path}') WHERE entity_type IN ('category', 'product') AND entity_uuid != '${categoryUUid}'`);
        }
    }
    catch (err) {
        error(err);
...
//# sourceMappingURL=builUrlRewrite.js.map