...
const deleteUrlReWrite = async (data) => {
    try {
        ...
        await execute(pool, `DELETE FROM url_rewrite WHERE entity_type = 'category' AND entity_uuid = '${categoryUuid}'`);
        if (!urlRewrite) {
            return;
        }
        else {
            // Delete all the url rewrite rule for the sub categories and products
            await execute(pool, `DELETE FROM url_rewrite WHERE request_path LIKE '${urlRewrite.request_path}/%' AND entity_type IN ('category', 'product')`);
        }
    }
    catch (err) {
        error(err);
...
//# sourceMappingURL=deleteUrlRewrite.js.map