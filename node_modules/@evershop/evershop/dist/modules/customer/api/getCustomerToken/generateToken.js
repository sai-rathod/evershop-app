...
export default async (request, response, next) => {
    try {
        ...
        response.status(OK);
        const accessToken = generateToken({ customer: request.locals.customer }, TOKEN_TYPES.CUSTOMER);
        const refreshToken = generateRefreshToken({ customer: request.locals.customer }, TOKEN_TYPES.CUSTOMER);
        response.json({
            data: {
                accessToken,
                refreshToken
