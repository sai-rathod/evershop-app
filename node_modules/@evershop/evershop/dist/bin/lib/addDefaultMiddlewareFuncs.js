...
export function addDefaultMiddlewareFuncs(app) {
    app.use((request, response, next) => {
        ...
        response.on('finish', () => {
            ...
            if (((_a = request.currentRoute) === null || _a === void 0 ? void 0 : _a.id) === 'staticAsset' ||
                ((_b = request.currentRoute) === null || _b === void 0 ? void 0 : _b.id) === 'adminStaticAsset') {
                return;
            }
            debug(message);
        });
    ...
    app.use((request, response, next) => {
        ...
        if (matchedRoutes.length > 1) {
            warning(`Multiple routes matched for ${requestPath}. Please check your routes: ${matchedRoutes
                .map((r) => r.id)
    ...
    const sessionMiddleware = (request, response, next) => {
        ...
        if (currentRoute === null || currentRoute === void 0 ? void 0 : currentRoute.isApi) {
            // We don't need session for api routes. Restful api should be stateless
            next();
        }
        else if (currentRoute === null || currentRoute === void 0 ? void 0 : currentRoute.isAdmin) {
    ...
    app.use(async (request, response, next) => {
        ...
        if (!route || !isBuildRequired(route)) {
            ...
            adminTailwindConfig.content = frontStoreTailwindConfig.content = [
                ...
                path.join(CONSTANTS.ROOTPATH, 'packages', 'evershop', 'dist', '**', '[A-Z]*.js'),
                // All file in node_modules/@evershop/evershop/dist and name is capitalized
                path.join(CONSTANTS.ROOTPATH, 'node_modules', '@evershop', 'evershop', 'dist', '**', '[A-Z]*.js'),
                ...enabledExtensions.map((extension) => path.join(extension.path, '**', '[A-Z]*.js'))
            ];
            const theme = getEnabledTheme();
            if (theme) {
                frontStoreTailwindConfig.content.push(path.join(theme.path, 'dist', '**', '[A-Z]*.js'));
            }
            if (!route.webpackCompiler) {
                route.webpackCompiler = webpack(createConfigClient(route, route.isAdmin ? adminTailwindConfig : frontStoreTailwindConfig));
            }
            ...
            if (!notFoundRoute.webpackCompiler) {
                notFoundRoute.webpackCompiler = webpack(createConfigClient(notFoundRoute, frontStoreTailwindConfig));
            }
            ...
            if (!adminNotFoundRoute.webpackCompiler) {
                adminNotFoundRoute.webpackCompiler = webpack(createConfigClient(adminNotFoundRoute, adminTailwindConfig));
            }
...
//# sourceMappingURL=addDefaultMiddlewareFuncs.js.map